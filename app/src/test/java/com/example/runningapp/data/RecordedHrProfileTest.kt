...
class RecordedHrProfileTest {
    ...
    fun `half a pair is nobody's Reserve, so it is not read as one`() {
        ...
        assertNull(run.copy(maxHrAtRun = 195).recordedHrProfile())
        assertNull(run.copy(restingHrAtRun = 48).recordedHrProfile())
    }
    ...
    fun `an unstated resting heart rate is a value, not a missing one`() {
        ...
        val recorded = run.copy(maxHrAtRun = 190, restingHrAtRun = 0).recordedHrProfile()
