...
class AiCoachClient {
    ...
    suspend fun evaluateProgress(context: AiTrainingContext): AiCoachResponse {
        ...
        val prompt = buildString {
            ...
            appendLine("- earlyBreakdownRatePercent")
            ...
            appendLine("- Repeated early breakdown suggests the run interval is too aggressive; consider shortening run duration and/or increasing walk support.")
            appendLine("- If clean performance is strong (for example >=80% completion quality) with stable or improving drift/recovery, consider safely extending run duration.")
