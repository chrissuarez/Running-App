...
/**
 * How a history drawn against the calendar steps and labels its bottom axis. Which points it holds
 * is the league table's to say ([LeagueTable.trend]).
 *
 * Written once because every trend chart asks it ([TrendLineChart]), and all of them are the same
 * picture of the same kind of history. Vico's own rule is what makes the axis arithmetic rather than
 * a preference, so a second copy of it would be a second chance to get Vico wrong.
 */
